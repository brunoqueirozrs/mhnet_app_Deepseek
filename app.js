/**
 * ============================================================
 * MHNET VENDAS - LÓGICA V122 (MASTER SYNC FINAL)
 * ============================================================
 * 📝 UPDATE:
 * - Sincronização total com Index V119 e Backend V110.
 * - Função 'verTodosLeads' global para o menu inferior.
 * - GPS captura Cidade.
 * - IA e Gestão de Admin ativas.
 * ============================================================
 */

// ⚠️ ID DO BACKEND V110
// Certifique-se de que este ID corresponde à sua última implantação do Backend V110
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("🚀 MHNET App V122 - Ready");
...
function filtrarPorStatus(status) {
    ...
    if (status !== 'Todos') {
        ...
        if(input) input.placeholder = "Buscar nome, bairro, telefone...";
    }
...
async function carregarLeads(showLoader = true) {
    ...
    // Se for admin, pede leads do admin (o backend deve retornar tudo)
    // Se for vendedor, pede leads do vendedor
...
function abrirLeadDetalhes(index) {
    ...
    if(l.agendamento) {
        ...
        if(parts.length >= 1) {
            ...
            if(dateParts.length === 3) document.getElementById('agendarData').value = `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;
        }
        if(parts.length >= 2) document.getElementById('agendarHora').value = parts[1];
    } else {
