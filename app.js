/**
 * ============================================================
 * MHNET VENDAS - LÓGICA V101 (FINAL + MATERIAIS VISUAIS)
 * ============================================================
 * 📝 UPDATE:
 * - Restaurado design de pastas (Ícones grandes).
 * - Restaurado design de imagens (Miniatura + Botões WhatsApp/Baixar).
 * ============================================================
 */

// ⚠️ ID DO BACKEND V93 (Mantenha o seu ID atual se estiver funcionando)
...
function initApp() {
    ...
    if (loggedUser === "Bruno Garcia Queiroz") document.getElementById('btnAdminSettings')?.classList.remove('hidden');
...
async function carregarMateriais(f=null, s="") {
    ...
    try {
        ...
        div.innerHTML = `<div class="col-span-2 text-center text-red-400 py-10"><i class="fas fa-wifi mb-2"></i><br>Falha ao carregar.<br><button onclick="carregarMateriais('${f || ''}')" class="mt-3 bg-blue-100 text-blue-600 px-4 py-2 rounded-lg text-sm font-bold">Tentar Novamente</button></div>`;
    }
}
...
function renderMateriais(items) {
    ...
    div.innerHTML = items.map(item => {
        if (item.type === 'folder') {
            ...
            // 🖼️ DESIGN DE IMAGEM (Miniatura + Botões)
            return `
...
// ... (RESTANTE DO CÓDIGO - MANTIDO DA V99) ...
