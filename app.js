/**
 * ============================================================
 * MHNET VENDAS - LÓGICA FRONTEND (v6.3 - Fix Login)
 * ============================================================
 */
...
document.addEventListener('DOMContentLoaded', () => {
  // Se já tem usuário, entra direto
  if (loggedUser) {
    initApp();
  } else {
    // Se não, mostra menu e carrega vendedores IMEDIATAMENTE
    showUserMenu();
    carregarVendedores();
  }
});
...
// *** VENDEDORES (Correção de Falha) ***
async function carregarVendedores() {
  const select = document.getElementById('userSelect');
  if (!select) return;

  // Lista de segurança (Hardcoded) para garantir acesso offline
  const listaSeguranca = [
      { id: 1, nome: "Ana Paula Rodrigues" },
      { id: 2, nome: "Vitoria Caroline Baldez Rosales" },
      { id: 3, nome: "João Vithor Sader" },
      { id: 4, nome: "João Paulo da Silva Santos" },
      { id: 5, nome: "Claudia Maria Semmler" },
      { id: 6, nome: "Diulia Vitoria Machado Borges" },
      { id: 7, nome: "Elton da Silva Rodrigo Gonçalves" }
  ];
  
  // Define fallback imediato caso a API demore
  const fallbackTimeout = setTimeout(() => {
      console.warn("API demorou. Usando lista local.");
      renderizarOpcoesVendedores(select, listaSeguranca);
  }, 3000); // 3 segundos timeout

  try {
    const res = await apiCall('getVendedores', {}, false, true); // showLoader=false para não travar tela
    ...
    clearTimeout(fallbackTimeout);
    console.error("Erro ao buscar vendedores. Usando lista local.", e);
    renderizarOpcoesVendedores(select, listaSeguranca);
  }
}
...
function renderizarOpcoesVendedores(selectElement, lista) {
  ...
  lista.forEach(v => {
    // Normaliza nome (API pode retornar .nome ou .Nome ou vetor)
             ...
             history.innerHTML = `
                <div class="flex gap-3">
                    ...
                    <div class="bg-white p-4 rounded-2xl rounded-tl-none border border-gray-100 text-sm text-gray-600 shadow-sm max-w-[80%]">
                        Olá! Sou o assistente MHNET. Posso ajudar com técnicas de venda ou detalhes dos planos.
                    </div>
                </div>`;
        }
    } else {
