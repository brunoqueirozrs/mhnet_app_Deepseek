/**
 * ============================================================
 * MHNET VENDAS - LÓGICA ORGANIZADA (v11.0 - Validação)
 * Estrutura dividida por telas para facilitar testes.
 * ============================================================
 */
...
function initApp() {
  ...
  setTimeout(() => carregarLeads(), 500); // Carrega dados em background
}
...
function navegarPara(pageId) {
  ...
  // Rola container principal para o topo
...
// ------------------------------------------------------------
// 4. PÁGINA: CADASTRO LEAD (Funcionalidade de Salvar)
// ------------------------------------------------------------
async function enviarLead() {
  ...
  // Tratamento de sucesso (mesmo com bloqueio visual de CORS)
...
// ------------------------------------------------------------
// 5. PÁGINA: GESTÃO LEADS (Lista e Histórico)
// ------------------------------------------------------------
async function carregarLeads() {
  ...
  if (res && res.status === 'success') {
    // Filtra apenas leads do usuário logado
    ...
    if(lista) lista.innerHTML = '<div style="text-align:center; color:#cbd5e1; padding:20px">Sem conexão para histórico.</div>';
  }
}
...
async function enviarMensagemChat() {
    ...
    const prompt = `Aja como um especialista comercial da MHNET. Responda: "${msg}"`;
...
async function apiCall(route, payload, show=true, suppress=false) {
  ...
  try {
    ...
    try { 
        ...
        // Se falhar JSON mas for POST, pode ser sucesso opaco (CORS)
    ...
    // Tratamento de falha silenciosa se for POST (assumimos envio)
